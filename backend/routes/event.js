...
const JWT_SECRET = "Kartikgoodb@oy";
...
router.post(
  ...
  [
    body(
      "categoryname",
      "The category name  should be at least 4 characters long"
    ).isLength({ min: 4 }),
  ],
  async (req, res) => {
    let success = false;
    ...
    if (category) {
      return res
        .status(400)
        .json({ error: "Sorry category is already exists" });
    }
    try {
...
router.post(
  ...
  [
    body(
      "sponserName",
      "The sponser name should be at least 4 characters long"
    ).isLength({ min: 4 }),
  ],
  async (req, res) => {
    let success = false;
...
router.put(
  ...
  async (req, res) => {
    ...
    try {
      ...
      const newSponser = {};
      if (sponserName) {
        newSponser.sponserName = sponserName;
      }
      if (sponserLogo) {
        newSponser.sponserLogo = sponserLogo;
      }
      if (sponserDetail) {
        newSponser.sponserDetail = sponserDetail;
      }
      ...
      success = true;
      sponser = await Sponser.findByIdAndUpdate(
        req.params.id,
        { $set: newSponser },
        { new: true }
      );
...
router.post(
  ...
  [
    body(
      "eventName",
      "Event name should be at least 4 characters long"
    ).isLength({ min: 4 }),
    body("contact", "Mobile number should contains 10 digits").isLength({
      min: 10,
      max: 10,
    }),
...
router.put(
  ...
  async (req, res) => {
    ...
    try {
      ...
      if (eventStDate) {
        newEvent.eventStDate = eventStDate;
      }
      if (eventEndDate) {
      ...
      if (noOfTicket) {
        newEvent.noOfTicket = noOfTicket;
      }
      if (totalPrice) {
...
router.post("/uploadeventimage/:id", fetchuser, async (req, res) => {
  ...
  try {
    const image = await Event.findByIdAndUpdate(req.params.id, {
      image: base64,
    });
    res.send(image);
  } catch (error) {
