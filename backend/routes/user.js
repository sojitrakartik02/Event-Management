...
router.post(
  ...
  [
    ...
    body(
      "password",
      "The password should be at least 8 characters long.The password should contain at least one uppercase letter, one lowercase letter, and one number, and one symbol."
    ).isStrongPassword({
      minLength: 8,
      minLowercase: 1,
      minUppercase: 1,
    ...
    body("contactno", "Mobile number should contains 10 digits").isLength({
      min: 10,
      max: 10,
    }),
  ],
...
router.put(
  ...
  async (req, res) => {
    ...
    try {
      const { fname, lname, email, country, contactno, gender, detail, image } =
        req.body;
      const newProfile = {};
      if (fname) {
